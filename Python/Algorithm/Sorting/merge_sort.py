def merge_sort(mylist):
    if len(mylist) == 1:
        return mylist
    return merge_two_sorted_list(merge_sort(mylist[0:len(mylist)//2]), merge_sort(mylist[len(mylist)//2: len(mylist)]))


def merge_two_sorted_list(sorted_list1, sorted_list2):
    sorted_list = []
